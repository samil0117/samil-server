// ⚠️ 이 파일은 렌더러 src/renderer/src/lib/dice/engine.ts 의 미러본입니다.
//    다이스 규칙 변경 시 양쪽을 함께 수정하세요. (추후 shared/ 통합 예정)
 ...
 * 반환 modifier: 순수 가산식(곱/나눗셈·키프드롭 없음)이면 합−주사위합(=상수항), 아니면 0(카드 표시 깔끔).
 * 흔한 주사위 표기를 폭넓게 수용하되 결과 모델(rolls/modifier/total)은 유지(기존 호출부 호환).
 */
 ...
 * 반드시 송신 시 1회만 해석(멀티=서버 권위)해야 모든 화면에 같은 숫자가 박제된다. 채팅·스크립트 공통.
 */
export function resolveInlineRolls(text: string, rng: RNG = defaultRng): string {
  if (!text.includes('[[')) return text
  return text.replace(/\[\[([^[\]]{1,60})\]\]/g, (full, raw: string) => {
    const expr = raw.trim()
    if (!/^[\d\s+\-*/dDkKhHlL()]+$/.test(expr) || !/\d*d\d+/i.test(expr)) return full // 주사위 식(안전 문자)만 치환
...
export function parseCommand(input: string, rng: RNG = defaultRng): DiceResult | null {
  ...
  if (/d\d/i.test(s) && /^[0-9dkhl+\-*/() ]+$/i.test(s)) return rollSum(s, { rng })
  return null
}
