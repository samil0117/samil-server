 ...
 * 8000 이면 data URL 이 잘려 닫는 ']' 가 사라지고 리터럴 텍스트로 표시되던 버그 → 상향.
 */
 ...
 * 클라 업로드 실패 폴백·구버전·공격 중 하나다. 저장·재브로드캐스트 전에 떨궈 이벤트 루프 정지·메모리 폭증
 * (=PL 튕김·재접속 불가·로그인 먹통)을 원천 차단한다. 참조('asset:')·유튜브 id·작은 인라인은 통과(false).
 */
