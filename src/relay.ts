...
export function createRelay(opts?: {
  ...
  const handler = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    // 백업 생성/메타 수정 — 토큰 인증. 본문 { token, log }. 새 백업은 html(외부화된 자기완결 문서) 필요, id 있으면 메타만.
    ...
    if (req.method === 'POST' && req.url === '/friend/list') {
      void readJsonBody(req).then((body) => {
        ...
        // 온라인 표시는 프레즌스(접속 중인 계정)로 덧붙인다.
        const withOnline = (arr: { id: string }[]): unknown[] =>
          arr.map((u) => ({ ...u, online: presence.has(u.id) }))
    ...
    if (req.method === 'POST' && req.url === '/admin/overview') {
      void readJsonBody(req).then((body) => {
        ...
        const per = accountsList.map((u) => {
          ...
          const refs = new Set<string>([...rooms.refs, ...chars.refs, ...blog.refs, ...slog.refs, ...prof.refs])
          return { u, rooms, chars, blog, slog, prof, refs }
        })
        ...
        const users = per.map(({ u, rooms, chars, blog, slog, prof, refs }) => {
          let assetLogical = 0
          let assetUnique = 0
          for (const h of refs) {
            const sz = sizeOf(h)
          ...
          const serialized = rooms.bytes + chars.bytes + blog.bytes + slog.bytes + prof.lobbyBytes + prof.profileBytes
          return {
            id: u.id,
            username: u.username,
            nickname: u.nickname,
        ...
        res.writeHead(200, JSON_H)
        res.end(JSON.stringify({ ok: true, summary, users }))
      })
      return
    }
    ...
    if (req.method === 'POST' && req.url === '/me/export') {
      void readJsonBody(req).then((body) => {
        ...
        try {
          ...
          res.writeHead(200, JSON_H)
          res.end(JSON.stringify({ type: 'tacoyaki-member-server', version: 1, blog, sessionlogs: slog, rooms, assets: assetsOut }))
        } catch (e) {
  ...
  return { httpServer, io, store, auth, characters, assets, dm, posts, sessionlogs }
}
