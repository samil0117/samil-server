...
import { GLOBAL_MAP_ID } from './protocol'
...
export function createRelay(opts?: {
  ...
  const handler = (req: IncomingMessage, res: ServerResponse): void => {
    ...
    if (req.method === 'POST' && req.url === '/admin/overview') {
      void readJsonBody(req).then((body) => {
        ...
        const users = per.map(({ u, rooms, chars, blog, slog, prof, dt, econ, refs }) => {
          ...
          const serialized = rooms.bytes + chars.bytes + blog.bytes + slog.bytes + prof.lobbyBytes + prof.profileBytes + dt.bytes + econ.bytes
          return {
            id: u.id,
            username: u.username,
            nickname: u.nickname,
    ...
    // 구버전 낚시 경로 — 미니게임 개편으로 폐지(조용한 실패 대신 업데이트 안내).
  ...
  io.on('connection', (socket) => {
    ...
    socket.on('chat:send', (req) => {
      ...
      // 비주얼 카드 트리거 — (주사위 외) 메시지 텍스트가 카드 이름과 정확히 일치하면 전원 오버레이 재생.
      if ('text' in message && message.text) {
        const card = store.findCardByTitle(roomId, message.text)
  ...
  return { httpServer, io, store, auth, characters, assets, dm, posts, sessionlogs, dottown, economy, market }
}
