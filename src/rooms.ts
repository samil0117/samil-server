...
/** 세션방 목록 항목(room:list). 카드·메타만 — 장면/채팅 본문은 입장 시 스냅샷으로. ⚠️ 클라 protocol 과 미러. */
...
export class RoomStore {
   ...
   * ⚠️ 예전엔 writeFileSync 로 대용량 방(맵 이미지·BGM base64 수백 MB)을 8초마다 통째로 동기 저장 → 이벤트 루프가
   *    수 초간 막혀 원격 PL 의 하트비트/HTTP 응답이 지연·타임아웃되며 반복적으로 튕기던 원인. 비동기 쓰기로 해소.
